...
void ICACHE_FLASH_ATTR oled_wirte(u8 addr,u8 data)
{

}

u8 ICACHE_FLASH_ATTR oled_read(u8 addr)
{
...
// IIC Write Command
**********************************************/
bool ICACHE_FLASH_ATTR Write_IIC_Command(u8 IIC_Command)
{
	uint8 ack;
	i2c_master_start();
...
void ICACHE_FLASH_ATTR OLED_WR_Byte(u8 dat,u8 cmd)
{
	if(cmd)
	{
		Write_IIC_Data(dat);
...
//m^n函数
u32 ICACHE_FLASH_ATTR oled_pow(u8 m,u8 n)
{
	u32 result=1;
	while(n--)result*=m;
	return result;
...
//num:数值(0~4294967295);
void ICACHE_FLASH_ATTR OLED_ShowNum(u8 x,u8 y,u32 num,u8 len,u8 size2)
{
	u8 t,temp;
	u8 enshow=0;
	for(t=0;t<len;t++)
...
//显示一个字符号串
void ICACHE_FLASH_ATTR OLED_ShowString(u8 x,u8 y,u8 *chr,u8 Char_Size)
{
	unsigned char j=0;
	while (chr[j]!='\0')
...
//显示汉字
void ICACHE_FLASH_ATTR OLED_ShowQmtzdst(u8 x,u8 y,u8 no)
{
	u8 t,adder=0;
	OLED_Set_Pos(x,y);
    for(t=0;t<12;t++)
...
/***********显示BMP图片，128x64起点坐标(x,y),x范围0~127，y为页的范围0~7*****************/
void ICACHE_FLASH_ATTR OLED_DrawBMP(unsigned char x0, unsigned char y0,unsigned char x1, unsigned char y1,unsigned char BMP[])
{
	unsigned int j=0;
	unsigned char x,y;
